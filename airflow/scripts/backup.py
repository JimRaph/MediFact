...
from utilities.s3_utils import upload_to_s3, download_from_s3, download_all_files, \
...
def extract_last_updated_and_hash(html: str) -> Dict[str, str]:
    """
    Extracts the 'timestamp' value from WHO fact sheet HTML
    and computes a SHA256 hash of the main content section.
    """
    soup = BeautifulSoup(html, "html.parser")

    timestamp_el = soup.find(class_="timestamp")
    last_updated = timestamp_el.get_text(strip=True) if timestamp_el else ""
...
def embed_and_upload_chroma(batch_size: int = 100):
    ...
    # Sync updated ChromaDB folder back to S3
